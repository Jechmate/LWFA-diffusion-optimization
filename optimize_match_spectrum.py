...
✓ Hybrid Bayesian + gradient-based optimization (LBFGS or Adam)
...
OPTIMIZERS:
-----------
✓ LBFGS: Second-order optimizer with line search (default)
...
import logging
from schedulefree import RAdamScheduleFree, ScheduleFreeWrapperReference
...
class SpectrumMatchingOptimizer:
    ...
    - LBFGS: Second-order optimizer with line search
    - Adam: First-order adaptive learning rate optimizer
    """
    def __init__(
        self,
        ...
        if bayesian_n_calls == 0:
            ...
            if self.finetune_optimizer == 'adam':
                ...
                print(f"  LBFGS: max {finetune_max_iter} iter, lr={finetune_lr}")
        else:
    ...
    def finetune_objective_and_grad(self, params_tensor):
        """Objective function with gradients for LBFGS fine-tuning phase."""
    ...
    def run_lbfgs_phase(self, initial_params):
        """Run the LBFGS fine-tuning phase."""
        print(f"Phase 2: LBFGS Fine-tuning ({self.finetune_max_iter} max iterations)")
        ...
        def closure():
            ...
            # Calculate metrics for logging and store spectrum data
            with torch.no_grad():
                settings = torch.stack([laser_energy, pressure, acquisition_time]).unsqueeze(0)
                x = self.sampler.sample_differentiable(
                    resolution=self.spectrum_length,
            ...
            # Store history with spectrum data
            ...
            # Log to file
            if self.logger:
                self.logger.info(f"LBFGS Step {iteration_count}: "
                               f"laser_energy={laser_energy.item():.6f}, pressure={pressure.item():.6f}, "
                               f"acquisition_time={acquisition_time.item():.6f}, MSE={spectrum_dist:.6f}")
            
            if iteration_count % 5 == 0:
                print(f"  LBFGS Iter {iteration_count}: [{laser_energy.item():.3f}, {pressure.item():.3f}, {acquisition_time.item():.3f}] -> MSE={spectrum_dist:.6f}")
    ...
    def run_adam_phase(self, initial_params):
        """Run the Adam optimization phase."""
        print(f"Phase 2: Adam Optimization ({self.finetune_max_iter} epochs)")
        ...
        for epoch in range(self.finetune_max_iter):
            optimizer.zero_grad()
            ...
            if current_objective < best_objective:
                best_objective = current_objective
                best_spectrum_dist = spectrum_dist
                best_params = [laser_energy.item(), pressure.item(), acquisition_time.item()]
            
            # Store history with spectrum data
            self.lbfgs_history.append({  # Using lbfgs_history for compatibility with plotting
                'iteration': epoch,
                'laser_energy': laser_energy.item(),
                'pressure': pressure.item(),
                'acquisition_time': acquisition_time.item(),
            ...
            # Log to file
            if self.logger:
                self.logger.info(f"Adam Epoch {epoch}: "
                               f"laser_energy={laser_energy.item():.6f}, pressure={pressure.item():.6f}, "
                               f"acquisition_time={acquisition_time.item():.6f}, MSE={spectrum_dist:.6f}")
            
            if epoch % 5 == 0 or epoch == self.finetune_max_iter - 1:
                print(f"  Adam Epoch {epoch}: [{laser_energy.item():.3f}, {pressure.item():.3f}, {acquisition_time.item():.3f}] -> MSE={spectrum_dist:.6f}")

        optimizer.eval()
        print(f"  Final: [{best_params[0]:.3f}, {best_params[1]:.3f}, {best_params[2]:.3f}] -> MSE={best_spectrum_dist:.6f}")
        return {
            'best_params': best_params, 
            'best_objective': best_objective,
            'best_spectrum_dist': best_spectrum_dist,
            'iterations': self.finetune_max_iter
        }

    def run_finetune_phase(self, initial_params):
        """Run the fine-tuning phase with the selected optimizer."""
        if self.finetune_optimizer == 'adam':
            return self.run_adam_phase(initial_params)
        else:
            return self.run_lbfgs_phase(initial_params)
    ...
    def plot_results(self):
        ...
        if lbfgs_objectives:
            ...
            ax.plot(combined_evals, lbfgs_best_so_far, 'r-', linewidth=2, label='LBFGS')
        ...
        for idx, (param_name, param_label) in enumerate(zip(param_names, param_labels)):
            ...
            if lbfgs_objectives:
                values_lbfgs = [call[param_name] for call in self.lbfgs_history]
                ...
                ax.plot(offset_iters, values_lbfgs, 'r.-', alpha=0.7, label='LBFGS')
...
def run_multi_seed_optimization():
    ...
    opt_params = {
        ...
        'device': device,
        'target_spectrum_csv': 'avg_spectrum_35_18_10.csv',
        'pressure_bounds': (1.0, 50.0),
        'laser_energy_bounds': (5.0, 50.0),
        'acquisition_time_bounds': (5.0, 100.0),
        'bayesian_n_calls': 100,
        ...
        'finetune_max_iter': 50,
        'finetune_lr': 2.0,
        'finetune_optimizer': 'lbfgs',  # Options: 'lbfgs' or 'adam'
        'batch_size': 16,
        'num_sampling_steps': 18,
        'sigma_min': 0.002,
        'sigma_max': 80,
...
def plot_multi_seed_comparison(all_results, all_optimizers, seeds, base_output_dir):
    ...
    for i, (result, optimizer) in enumerate(zip(all_results, all_optimizers)):
        ...
        if optimizer.lbfgs_history:
            ...
            combined_evals = [offset + i for i in range(len(lbfgs_objectives))]
            ax1.plot(combined_evals, lbfgs_objectives, '--', color=color, alpha=0.7, linewidth=1.5)
    ...
    for param_name, param_label, (row, col) in zip(param_names, param_labels, plot_positions):
        ...
        for i, optimizer in enumerate(all_optimizers):
            ...
            if optimizer.lbfgs_history:
                values_lbfgs = [call[param_name] for call in optimizer.lbfgs_history]
                iters = [call['iteration'] for call in optimizer.lbfgs_history]
                offset = len(values_bayes) if optimizer.bayesian_history else 0
                offset_iters = [offset + iter_num for iter_num in iters]
                ax.plot(offset_iters, values_lbfgs, '--', color=color, alpha=0.6, linewidth=1)
