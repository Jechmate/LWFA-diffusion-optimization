...
class SpectrumWeightedSumOptimizer:
    ...
    def _optimize_single_run(self):
        ...
        if self.logger is None:
            method_name = "EDM_Adam_Cosine_Annealing"
        ...
        # Create Adam optimizer for parameters
        ...
        progress_data = {
            'method': 'EDM_Adam_Cosine_Annealing',
        ...
        # Run optimization using Adam with cosine annealing
        for step in tqdm(range(self.optimization_steps), desc="Optimizing weighted sum with Adam + cosine annealing"):
        ...
        if self.trial_num is not None:
            save_optimization_progress(self.output_dir, "EDM_Adam_Cosine_Annealing", 
...
def compare_optimization_methods(edm_optimizer, bayesian_optimizer, output_dir="comparison_results"):
    """
    Compare the results of EDM Adam optimization vs Bayesian optimization.
    """
    ...
    axs[0, 0].plot(edm_weighted_sums, 'b-', linewidth=2, label='EDM Adam-based', alpha=0.8)
    ...
    print(f"EDM Adam Optimization Results ({len(edm_weighted_sums)} trials):")
    ...
    if bayesian_best > edm_best:
        ...
        print(f"  EDM Adam achieved {improvement:.2f}% better result")
    else:
...
def run_multi_trial_comparison(
    ...
    print("\n" + "="*50)
    print("RUNNING EDM ADAM OPTIMIZATION TRIALS")
    print("="*50)
...
def analyze_multi_trial_results(edm_results, bayesian_results, output_dir):
    ...
    print(f"EDM Adam Optimization Results ({len(edm_results)} trials):")
...
def run_comprehensive_optimization_comparison(
    ...
    edm_config = {
        "EDM_Adam_Cosine_Annealing": {
            "use_cosine_annealing": True,
            "T_max": 50,  # Match optimization_steps
            "eta_min": 1e-6,
            "description": "EDM optimization with Adam and cosine annealing scheduler"
        }
    }
    ...
    config_name = "EDM_Adam_Cosine_Annealing"
...
def analyze_comprehensive_results(all_results, output_dir):
    ...
    baseline_scores = method_stats.get('EDM_Adam_Cosine_Annealing', {}).get('scores', [])
    if baseline_scores:
        print(f"\nComparisons with EDM Adam Cosine Annealing:")
        print(f"{'Method':<25} {'Improvement':<12} {'p-value':<10} {'Significance':<12}")
        print("-" * 65)
        
        for method_name, method_stat in method_stats.items():
            if method_name == 'EDM_Adam_Cosine_Annealing':
                continue
...
def main():
    ...
    print("="*80)
    print("EDM ADAM COSINE ANNEALING vs BAYESIAN OPTIMIZATION COMPARISON")
    print("="*80)
    print("This will run a complete comparison of:")
    print("  • EDM with Adam + Cosine Annealing")
    ...
    print()
    print("Adam + Cosine Annealing Details:")
    ...
    edm_mean = method_means.get('EDM_Adam_Cosine_Annealing', 0)
    bayesian_mean = method_means.get('Bayesian_Optimization', 0)
    
    print(f"📊 EDM Adam Cosine Annealing: {edm_mean:.4f}")
    ...
    if edm_mean > 0 and bayesian_mean > 0:
        if edm_mean >= bayesian_mean:
            ...
            print(f"📈 EDM Adam Cosine Annealing is {improvement:.1f}% better")
        else:
    ...
    print("="*80)
    print("EDM ADAM COSINE ANNEALING:")
    print("• Adam optimizer with adaptive moment estimation")
